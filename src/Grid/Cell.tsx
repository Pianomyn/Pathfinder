...
function Cell({
  ...
  const [previousSourceCellId, setPreviousSourceCellId] =
    useState<CellId | null>(null);
  const [previousTargetCellId, setPreviousTargetCellId] =
    useState<CellId | null>(null);
  useEffect(() => {
    ...
    <td
      ...
      onClick={() => {
        ...
        if (
          ...
          ALL_COLOR_MAPPINGS[cellTypeAsString] ==
          PLACEABLE_COLOR_MAPPINGS.Target
        ) {
          ...
          setCellColor(newCellType);
          var currentNode = graph.getNode(cellId);
          if (newCellType == ALL_COLOR_MAPPINGS.Unvisited) {
